  ...
  <div id="table-container" class="table-container">
      ...
      <el-form id="searchForm" :size="searchFormSize" inline :label-width="searchFormLabelWidth">
        ...
        <el-form-item v-for="item in searchHandle" :key="item.label">
          <el-button
            ...
            :style="item.style"
            :class="item.class"
            :disabled="(item.label==='Add New' || item.label==='Delete') ? isReadOnly : false"
    ...
    <section>
      ...
      >
        ...
        <el-table-column
          ...
          :align="selectionCol.align"
          :width="selectionCol.width"
          :selectable="!selectionCol.rowSelectAble?null:() => selectionCol.rowSelectAble(that.isReadOnly)"
          :render-header="isRenderHeader ? renderHeader : null"
        />
        <!-- 索引列 -->
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <span v-if="item.type==='button'">
              ...
              >
                <i
                  v-if="btn.isUseIcon"
                  v-show="(that.isReadOnly===false && btn.style.includes('pointer')) || (that.isReadOnly===true && btn.style.includes('not-allowed'))"
                  :disabled="isReadOnly"
                  :style="btn.style"
                  :class="btn.iconClass"
                  :title="btn.label"
                ...
                >{{ btn.label }}</el-button>
                <span
                  v-show="(that.isReadOnly===false && btn.style.includes('pointer') && (i < item.btnList.length-3)) || (that.isReadOnly===true && btn.style.includes('not-allowed') && (i < item.btnList.length-3))"
    ...
    <section>
      ...
      >
        <!-- 索引列 -->
        <el-table-column
          v-if="subIndexCol!==null"
          v-show="showIndex"
          type="index"
          :label="subIndexCol.label"
          :align="subIndexCol.align"
          :width="subIndexCol.width"
          :render-header="isRenderHeader ? renderHeader : null"
        />
        <!-- 数据栏 -->
        <el-table-column
          v-for="item in subVisibleTableCols"
          :key="item.id"
          :prop="item.prop"
          :label="item.label"
          :width="item.width"
...
export default {
  ...
  props: {
    ...
    // 子表相关属性
    subTableData: { type: Array, default: () => [] }, // 表格数据
    subTableCols: { type: Array, default: () => [] }, // 表格列配置
    subIndexCol: { type: Object, default: () => null } // 表格索引列
  },
  data() {
    return {
      tableHeight: 0,
  ...
  computed: {
    ...
    subVisibleTableCols: { // 显示的表格列
      get() {
        var tableCols = this.subTableCols.filter(c => isNullOrUndefined(c.visible) || c.visible === true || ((typeof c.visible) === 'function' && c.visible(this.that)))
        if (this.isRenderHeader) {
          const columns = calcColumnsWidth(tableCols, this.subTableData)
          tableCols = columns
        }
        return tableCols
      }
