  ...
  <div id="table-container" class="table-container">
    ...
    <section>
      ...
      <el-table
        ...
        :height="subOneTableHeight"
        :header-cell-style="{background:'#D2E4FF',color:'#606266'}"
        :data="subOneTableData"
        :size="tableSize"
        :border="isBorder"
      >
        <!-- 索引列 -->
        <el-table-column
          v-if="subOneIndexCol!==null"
          v-show="showIndex"
          type="index"
          :label="subOneIndexCol.label"
          :align="subOneIndexCol.align"
          :width="subOneIndexCol.width"
          :render-header="isRenderHeader ? renderHeader : null"
        />
        <!-- 数据栏 -->
        <el-table-column
          v-for="item in subOneVisibleTableCols"
          :key="item.id"
          :prop="item.prop"
          :label="item.label"
          :width="item.width"
      ...
      <el-table
        ...
        :style="tableStyle"
        style="float:left;width:50%"
        :height="subTwoTableHeight"
        :header-cell-style="{background:'#D2E4FF',color:'#606266'}"
        :data="subTwoTableData"
        :size="tableSize"
        :border="isBorder"
      >
        <!-- 索引列 -->
        <el-table-column
          v-if="subTwoIndexCol!==null"
          v-show="showIndex"
          type="index"
          :label="subTwoIndexCol.label"
          :align="subTwoIndexCol.align"
          :width="subTwoIndexCol.width"
          :render-header="isRenderHeader ? renderHeader : null"
        />
        <!-- 数据栏 -->
        <el-table-column
          v-for="item in subTwoVisibleTableCols"
          :key="item.id"
          :prop="item.prop"
          :label="item.label"
          :width="item.width"
    ...
    <!-- 分页 -->
    <section v-if="isPagination" ref="pageSection" class="table-footer-padding">
      <el-pagination
...
export default {
  ...
  props: {
    ...
    // 子表1相关属性
    subOneTableData: { type: Array, default: () => [] }, // 表格数据
    subOneTableCols: { type: Array, default: () => [] }, // 表格列配置
    subOneIndexCol: { type: Object, default: () => null }, // 表格索引列
    // 子表2相关属性
    subTwoTableData: { type: Array, default: () => [] }, // 表格数据
    subTwoTableCols: { type: Array, default: () => [] }, // 表格列配置
    subTwoIndexCol: { type: Object, default: () => null } // 表格索引列
  },
  data() {
    return {
      tableHeight: 0,
  ...
  computed: {
    visibleTableCols: { // 显示的表格列
      get() {
        var tableCols = this.tableCols.filter(c => isNullOrUndefined(c.visible) || c.visible === true || ((typeof c.visible) === 'function' && c.visible(this.that)))
        if (this.isRenderHeader) {
          const columns = calcColumnsWidth(tableCols, this.tableData)
          tableCols = columns
        }
        return tableCols
      }
    },
    subOneVisibleTableCols: { // 显示的表格列
      get() {
        var subOneTableCols = this.subOneTableCols.filter(c => isNullOrUndefined(c.visible) || c.visible === true || ((typeof c.visible) === 'function' && c.visible(this.that)))
        if (this.isRenderHeader) {
          const columns = calcColumnsWidth(subOneTableCols, this.subOneTableData)
          subOneTableCols = columns
        }
        return subOneTableCols
      }
    },
    subTwoVisibleTableCols: { // 显示的表格列
      get() {
        var subTwoTableCols = this.subTwoTableCols.filter(c => isNullOrUndefined(c.visible) || c.visible === true || ((typeof c.visible) === 'function' && c.visible(this.that)))
        if (this.isRenderHeader) {
          const columns = calcColumnsWidth(subTwoTableCols, this.subTwoTableData)
          subTwoTableCols = columns
        }
        return subTwoTableCols
      }
    }
  },
  watch: {
  ...
  methods: {
    ...
    setTableHeight() {
      this.$nextTick(function() {
        ...
        if (!isNullOrUndefined(this.$refs.pageSection)) {
          pageSetionHeight = this.$refs.pageSection.offsetHeight
        }
        this.tableHeight = window.innerHeight - searchSetionHeight - btnSetionHeight - pageSetionHeight - this.tableHeightOffset
        this.subOneTableHeight = 180
        this.subTwoTableHeight = 180
...
//   const padding = 0 // 单元格左右padding距离
//   let width = 0
//   const span = document.createElement('span')
//   span.innerText = str
...
// function getMaxLength(arr, minwidth = 152) {
