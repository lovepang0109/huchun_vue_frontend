...
export default {  
  ...
  computed: {
    ...mapGetters(['GetClearingDateData', 'GetReportMccMnc', 'IsLoading', 'GetError']),    
    getMasterProviders: function(){
      return this.GetClearingDateData?.masterProviderList?.map( item => `${item.NOMBRE} [${item.ID_MASTER}]`);      
    },    
  },
  mounted() {
    ...
    this.fetchClearingDateData({  
        cFechaInicio:   startDate,
        cFechaFin:      endDate,
        iIDMasterBulk: 1,        
      }).then(() => {
        this.connectionData = this.GetClearingDateData?.data;
        this.masterProviderModel = this.getMasterProviders[0];
      });     
  },
  methods: {
    ...mapActions(['fetchClearingDateData', 'fetchMccMncData']),    
    search() {
