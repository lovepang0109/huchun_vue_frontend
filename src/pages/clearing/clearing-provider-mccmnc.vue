...
export default {  
  ...
  computed: {
    ...mapGetters(['GetClearingProviderMccMncData', 'GetReportMccMnc', 'IsLoading', 'GetError']),    
    getMasterProviders: function(){
      return this.GetClearingProviderMccMncData?.masterProviderList?.map( item => `${item.NOMBRE} [${item.ID_MASTER}]`);      
    },    
  },
  mounted() {
    ...
    this.fetchClearingProviderMccMncData({  
        cFechaInicio:   startDate,
        cFechaFin:      endDate,
        iIDMasterBulk: 1,        
      }).then(() => {
        this.connectionData = this.GetClearingProviderMccMncData?.data;
        this.masterProviderModel = this.getMasterProviders[0];
      });     
  },
  methods: {
    ...mapActions(['fetchClearingProviderMccMncData', 'fetchMccMncData']),    
    search() {
