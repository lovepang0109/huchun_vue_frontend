...
export default {  
  ...
  computed: {
    ...mapGetters(['GetClearingProviderDateData', 'GetReportMccMnc', 'IsLoading', 'GetError']),    
    getMasterProviders: function(){
      return this.GetClearingProviderDateData?.masterProviderList?.map( item => `${item.NOMBRE} [${item.ID_MASTER}]`);      
    },    
  },
  mounted() {
    ...
    this.fetchClearingProviderDateData({  
        cFechaInicio:   startDate,
        cFechaFin:      endDate,
        iIDMasterBulk: 1,        
      }).then(() => {
        this.connectionData = this.GetClearingProviderDateData?.data;
        this.masterProviderModel = this.getMasterProviders[0];
      });     
  },
  methods: {
    ...mapActions(['fetchClearingProviderDateData', 'fetchMccMncData']),    
    search() {
