...
export default {  
  ...
  computed: {
    ...mapGetters(['GetBillingCustomerDateData', 'GetReportMccMnc', 'IsLoading', 'GetError']),
    getCountries: function(){
      return this.GetBillingCustomerDateData?.countryList?.map( item => {
        return {
          code: item.code,
          state: item.state,
    ...
    getTypeRoutes: function(){      
      const routes = this.GetBillingCustomerDateData?.typeRoute || {};
      return Object.keys(routes)?.map( route => `${routes[route]} [${route.substr(1,1)}]`);
    }, 
    getCustomers: function(){
      const customerList = this.GetBillingCustomerDateData?.customerList;
      if(!customerList) return [];
      return Object.keys(customerList)?.map( name => `${customerList[name].EMPRESA} [${customerList[name].ID_CLIENTE}]`);
    },     
    getProviders: function(){
      return this.GetBillingCustomerDateData?.providerList?.map( item => `${item.NOMBRE} [${item.ID_PROVEEDOR}]`);      
    },    
    getMccMnc: function(){
  ...
  mounted() {
    ...
    this.fetchBillingCustomerDateData({  
        cFechaInicio:   startDate,
        cFechaFin:      endDate,        
      }).then(() => {
        this.connectionData = this.GetBillingCustomerDateData?.data;
      });     
  },
  methods: {
    ...mapActions(['fetchBillingCustomerDateData', 'fetchMccMncData']),    
    search() {
      this.fetchBillingCustomerDateData({  
