...
export default {  
  ...
  data() {    
    return {
      ...
      headers: [
          ...
          { title: 'Customer',            align: 'start', key: 'Cliente', },
          ...
          { title: 'Price: Average',      align: 'start', key: 'Precio_Cliente' },            
          { title: 'Price: Total',        align: 'start', key: 'Total_Cliente' },            
      ], 
      countryModel: null,
      providerModel: null,
      customerModel: null,
  ...
  computed: {
    ...mapGetters(['GetBillingCustomerMccMncData', 'GetReportMccMnc', 'IsLoading', 'GetError']),
    getCountries: function(){
      return this.GetBillingCustomerMccMncData?.countryList?.map( item => {
        return {
          code: item.code,
          state: item.state,
    ...
    getTypeRoutes: function(){      
      const routes = this.GetBillingCustomerMccMncData?.typeRoute || {};
      return Object.keys(routes)?.map( route => `${routes[route]} [${route.substr(1,1)}]`);
    }, 
    getCustomers: function(){
      const customerList = this.GetBillingCustomerMccMncData?.customerList;
      if(!customerList) return [];
      return Object.keys(customerList)?.map( name => `${customerList[name].EMPRESA} [${customerList[name].ID_CLIENTE}]`);
    },     
    getProviders: function(){
      return this.GetBillingCustomerMccMncData?.providerList?.map( item => `${item.NOMBRE} [${item.ID_PROVEEDOR}]`);      
    },    
    getMccMnc: function(){
  ...
  mounted() {
    ...
    this.fetchBillingCustomerMccMncData({  
        cFechaInicio:   startDate,
        cFechaFin:      endDate,        
      }).then(() => {
        this.connectionData = this.GetBillingCustomerMccMncData?.data;
      });     
  },
  methods: {
    ...mapActions(['fetchBillingCustomerMccMncData', 'fetchMccMncData']),    
    search() {
      this.fetchBillingCustomerMccMncData({  
