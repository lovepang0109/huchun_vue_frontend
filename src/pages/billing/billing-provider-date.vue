...
export default {  
  ...
  computed: {
    ...mapGetters(['GetBillingProviderDateData', 'GetReportMccMnc', 'IsLoading', 'GetError']),
    getCountries: function(){
      return this.GetBillingProviderDateData?.countryList?.map( item => {
        return {
          code: item.code,
          state: item.state,
    ...
    getTypeRoutes: function(){      
      const routes = this.GetBillingProviderDateData?.typeRoute || {};
      return Object.keys(routes)?.map( route => `${routes[route]} [${route.substr(1,1)}]`);
    }, 
    getCustomers: function(){
      const customerList = this.GetBillingProviderDateData?.customerList;
      if(!customerList) return [];
      return Object.keys(customerList)?.map( name => `${customerList[name].EMPRESA} [${customerList[name].ID_CLIENTE}]`);
    },     
    getProviders: function(){
      return this.GetBillingProviderDateData?.providerList?.map( item => `${item.NOMBRE} [${item.ID_PROVEEDOR}]`);      
    },    
    getMccMnc: function(){
  ...
  mounted() {
    ...
    this.fetchBillingProviderDateData({  
        cFechaInicio:   startDate,
        cFechaFin:      endDate,        
      }).then(() => {
        this.connectionData = this.GetBillingProviderDateData?.data;
      });     
  },
  methods: {
    ...mapActions(['fetchBillingProviderDateData', 'fetchMccMncData']),    
    search() {
      this.fetchBillingProviderDateData({  
