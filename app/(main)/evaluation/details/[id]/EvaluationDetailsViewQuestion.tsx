...
const EvaluationDetailsViewQuestion = ({
  test,
  setTest,
  questions,
  setQuestions,
  ...
  useEffect(() => {
    ...
    subscription = EvaluationService().evaluatedItem$.subscribe((item: any) => {
      const qidx = pendingQuestions.findIndex((q) => q._id == item.question);
      if (qidx > -1) {
        const eidx = pendingQuestions[qidx].evaluations.findIndex(
          (e) => e.QAId == item.QAId
        );
        if (eidx > -1) {
          const updatedQuestions = [...pendingQuestions];
          updatedQuestions[qidx].evaluations[eidx].evaluated = true;
          updatedQuestions[qidx].evaluations.splice(eidx, 1);
          setPendingQuestions(updatedQuestions);
        }

        if (!pendingQuestions[qidx].evaluations.length) {
          // reset evaluatedQuestions
          if (evaluatedQuestions) {
            const update_evaluatedQuestions = evaluatedQuestions;
            update_evaluatedQuestions.push(pendingQuestions[qidx]);
            setEvaluatedQuestions(update_evaluatedQuestions);
          }
          const update_pendingQ = pendingQuestions;
          update_pendingQ.splice(qidx, 1);
          setPendingQuestions(update_pendingQ);
          setTest({
            ...test,
            evaluatedQ: test.evaluatedQ + 1,
            pendingQ: test.pendingQ - 1,
          });
        }
      }
  ...
  const buildQuestionData = (test: any, evaluations: any) => {
    ...
    evaluations.forEach((evl) => {
      if (!evl.teacherComment) {
        evl.teacherComment = "";
      }
    });

    // assign evaluation items to each question
    test.questions.forEach((q) => {
      q.evaluations = evaluations.filter((e) => e.question == q._id);
    });

    return test.questions;
  };

  const evaluate = (q: any, ev: any, index?: any) => {
    if (ev.evaluating) {
      return;
    }
    if (ev.obtainMarks > q.plusMark || ev.obtainMarks < q.minusMark) {
      alert("Message", "Invalid mark");
      return;
    }
    if (ev.obtainMarks == null) {
      alert("Message", "Invalid mark");
      return;
    }
    ...
    ev.evaluating = true;
    evalSvc
      .questionEvaluation(ev.attemptId, {
        question: ev.question,
        status: 5,
        teacherComment: ev.teacherComment,
        marks: ev.obtainMarks,
        eId: ev.QAId,
      })
      .then((res) => {
        success("Question is evaluated successfully!");
        if (isPending) {
          const temp = q.evaluations.filter(
            (e) => e.attemptId !== ev.attemptId
          );
          q.evaluations = temp;
          const questions_temp = questions;
        ...
        EvaluationService().notifyEvaluated(
          ev.question,
          ev.student._id,
          ev.QAId
        );
        // setSelectedQuestion(prevState => {
        //   const updatedEvaluations = [...prevState.evaluations];
        //   updatedEvaluations[index].obtainMarks = '0';
        //   updatedEvaluations[index].teacherComment = ''
        //   return { ...prevState, evaluations: updatedEvaluations };
        // });
        const query: any = {};
  ...
  const onPendingToggle = () => {
    if (isPending) {
      if (pendingQuestions) {
        ...
        evalSvc
          .getQuestionsForEvaluationByTest(test._id, query)
          .then((res: any) => {
            setPendingQuestions(buildQuestionData(res.test, res.evaluations));
            setQuestions(buildQuestionData(res.test, res.evaluations));

            if (buildQuestionData(res.test, res.evaluations).length) {
              setSelectedQuestion(
                buildQuestionData(res.test, res.evaluations)[0]
              );
            } else {
              setSelectedQuestion(null);
            }
      ...
      if (evaluatedQuestions.length > 0) {
        ...
        if (questions.length) {
          setSelectedQuestion(evaluatedQuestions[0]);
        } else {
          setSelectedQuestion(null);
        }
      } else {
        ...
        evalSvc
          .getQuestionsForEvaluationByTest(test._id, query)
          .then((res: any) => {
            setEvaluatedQuestions(buildQuestionData(res.test, res.evaluations));
            setQuestions(buildQuestionData(res.test, res.evaluations));
  ...
  const loadMore = () => {
    ...
    evalSvc
      ...
      .then((res: any) => {
        if (isPending) {
          const update_pendingQuestions = [
            ...pendingQuestions,
            ...buildQuestionData(res.test, res.evaluations),
          ];
          setPendingQuestions(update_pendingQuestions);
          setQuestions(evaluatedQuestions);
        } else {
          const update_evaluatedQuestions = [
            ...evaluatedQuestions,
            ...buildQuestionData(res.test, res.evaluations),
          ];
          setQuestions(update_evaluatedQuestions);
        }
      });
  };
  ...
  const handleMarksChange = (e, index) => {
    ...
    setSelectedQuestion((prevState) => {
      const updatedEvaluations = [...prevState.evaluations];
      updatedEvaluations[index].obtainMarks = value;
      return { ...prevState, evaluations: updatedEvaluations };
    });
  };
  ...
  const handleCommentChange = (newComment, index) => {
    setSelectedQuestion((prevState) => {
      const updatedEvaluations = [...prevState.evaluations];
      updatedEvaluations[index].teacherComment = newComment;
      return { ...prevState, evaluations: updatedEvaluations };
    });
  };
  ...
  return (
    <>
      <div className="row">
        <div className="col-lg-3 eval-left-quesWise">
                  ...
                  <li key={question._id}>
                    <a
                      className={`text-truncate ${
                        question === selectedQuestion ? "active" : ""
                      }`}
                      onClick={() => setSelectedQuestion(question)}
                    >
                      <h4 className="f-14 text-black mt-0 float-none text-truncate">
                        Ques {question.order}.{" "}
                        {stripHtmlTags(question.questionText)}
                      </h4>
                      <p className="f-12 line-height-initial">
                        {question.evaluations.length}{" "}
        ...
        {questions ? (
          <div className="eval col-lg-9 eval-right-quesWise">
            {selectedQuestion && (
              <>
                <div className="error-analysis-area mw-100">
                            ...
                            <table className="table mb-0">
                              <tr>
                                <td className="border-0 p-0">
                                  <h6>Ques {selectedQuestion.order}.</h6>
                                </td>

                                <td className="border-0 p-0">
                                  <div className="ques eval-rightQuesText min-w-100">
                                    ...
                                    {selectedQuestion.audioFiles && (
                                      <div className="row">
                                        {selectedQuestion.audioFiles.map(
                                          (i, audio) => {
                                            <div className="position-relative my-2 col-lg-6 col-12">
                ...
                {selectedQuestion.evaluations.map((evalu, i) => (
                  <div className="error-analysis-area" key={evalu._id}>
                    <div className="accordation-area pt-0">
                      <div className="accordion" id={`accordion_${evalu._id}`}>
                        <div className="area mb-3 bg-white p-3">
                          <div id={`heading_${evalu._id}`}>
                            <h2>
                              <button
                                className={`btn-accordion-header active border-0 text-left p-0 w-100 collapsed ${
                                  evalu.evaluated ? "active" : ""
                                }`}
                                type="button"
                                data-toggle="collapse"
                                data-target={`#collapseContent_${evalu._id}`}
                                aria-expanded="true"
                                aria-controls={`collapseContent_${evalu._id}`}
                              >
                                <div className="form-row">
                                  <div className="col-auto">
                                    <div className="number_box number-remove-eval-rightQuesIndex-remove-custom-ColorQuestion px-0">
                                      <span>{i + 1}</span>
                                    </div>
                                  </div>
                                  <div className="col ">
                          ...
                          <div
                            id={`collapseContent_${evalu._id}`}
                            className="collapse"
                            aria-labelledby={`heading_${evalu._id}`}
                            data-parent={`#accordion_${evalu._id}`}
                          >
                            <div className="selection-wrap evalMarksAssign-expand p-0">
                              {evalu.answers && evalu.answers[0] && (
                                <div style={{ paddingLeft: "3.2rem" }}>
                                  <span className="text-dark">
                                    {evalu.answers[0].answerText && (
                                      <MathJax
                                        value={evalu.answers[0].answerText}
                                      ></MathJax>
                                    )}
                                  </span>

                                  {evalu.answers[0].attachments && (
                                    <div>
                                      {evalu.answers[0].attachments.map(
                                        (att, index) => (
                                          <div key={index}>
                                            {att.type === "image" && (
                                              <img
                              ...
                              <hr />
                              {!evalu.evaluated || settings.allowMarksChange ? (
                                <form
                                  className="form-boxes"
                                  onSubmit={(e) => {
                                    e.preventDefault();
                                    evaluate(selectedQuestion, evalu, i);
                                  }}
                                >
                                  <div>
                                    ...
                                    <div className="col-lg-6">
                                      <div className="row customizedSpanMarks-Assign mt-2">
                                        <h4 className="form-box_title d-inline-block mr-3">
                                          Enter marks
                                        </h4>
                                        <div className="customizedBorder-Input">
                                          <span>
                                            <input
                                              type="number"
                                              className="form-control border enter-MarksField"
                                              name="mark"
                                              placeholder=""
                                              value={evalu.obtainMarks}
                                              onChange={(e) =>
                                                handleMarksChange(e, i)
                                              }
                                              required
                                              max={selectedQuestion.plusMark}
                                              min={selectedQuestion.minusMark}
                                              step="0.5"
                                            />
                                          </span>
                                        </div>
                                  ...
                                  <div className="assignment mx-auto">
                                    <div className="assignment-content-area py-0">
                                      <div className="bg-white my-3">
                                        <CKEditorCustomized
                                          defaultValue={evalu.teacherComment}
                                  ...
                                  <div className="assignment mx-auto">
                                    <div className="assignment-content-area pt-0">
                                      <div className="bg-white mb-3">
                                        <MathJax value={evalu.teacherComment} />
                                      </div>
                                    </div>
                                  </div>
                                </div>
...
export default EvaluationDetailsViewQuestion;
