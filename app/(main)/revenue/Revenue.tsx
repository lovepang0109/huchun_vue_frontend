...
const Revenue = () => {
  ...
  const [intervalSoldData, setIntervalSoldData] = useState<number>(0);
  const [total, setTotal] = useState<number>(0);
  const [logs, setLogs] = useState<any>([]);
  const [allTransctionLogs, setAllTransctionLogs] = useState<any>([]);
  ...
  const [intervalRevenueData, setIntervalRevenueData] = useState<number>(0);
  ...
  const getSoldData = () => {
    publisherService.getSoldData().then((data: any) => {
      setTotal(
        (
          data.testseries.revenue +
          data.course.revenue +
          data.assessment.revenue
        ).toFixed(1)
      );
      setSold(
        (
          data.testseries.sold +
          data.course.sold +
          data.assessment.sold
        ).toFixed(1)
      );
    });
  };
  ...
  const getIntervalSoldData = () => {
    publisherService.getSoldData({ interval: 15 }).then((data: any) => {
      setIntervalSoldData(
        (
          data.testseries.sold +
          data.course.sold +
  ...
  const getTransactionLogs = (para?: any) => {
    if (!para) {
      para = params;
    }
    publisherService.getTransactionLogs(para).then((data: any) => {
      setLogs(data);
    });
  };
  ...
  const search = (text: string) => {
    ...
    publisherService
      .getTransactionLogs({ ...para, title: text })
      .then((data: any) => {
        setLogs(data);
      });
  };
  ...
  const filter = (type: string) => {
    if (type == "all") {
      ...
      getTransactionLogs(para);
      getTransactionLogsCount(para);
    } else if (type == "course") {
      setParams({
        page: 1,
        limit: 10,
  ...
  const getTransactionLogsCount = (para: any) => {
    if (!para) {
      para = params;
    }
    publisherService
      .getTransactionLogs({ ...para, count: true })
      .then((data: any) => {
  ...
  const excelExport = () => {
    const csvFileName = slugify(`revenue report ${user.name}`) + ".csv";

    const dataPush = [["Name", "Username", "Content Type", "Date", "Amount"]];

    publisherService
      .getTransactionLogs({ ...params, all: true })
      .then((data: any[]) => {
        for (const d of data) {
          dataPush.push([
            d.name,
            d.userId,
            d.type == "practice" ? "assessment" : d.type,
            d.createdAt,
            d.price,
          ]);
        }
      ...
      <div className="container">
        <div className="bg-white overall-main">
          <div className="section_heading_wrapper">
            <h3 className="section_top_heading">Overall Performance</h3>
          </div>
          <div className="data_ana_box">
            <div className="form-row align-items-center">
              <div className="col-md-4">
                <div className="overall-head">Total revenue earned</div>
                <div className="over3 d-flex justify-content-center">
                  <h1 className="big_num over2-inactive">{total}</h1>
              ...
              <div className="col-md-4">
                <img
                  className="img-fluid max-height-250px mx-auto my-3"
                  alt="publisher_revenue"
                  src="assets/images/REQAWA.svg"
                />
              </div>
              <div className="col-md-4">
                <div className="overall-head">Total publications sold</div>
                <div className="over3 d-flex justify-content-center">
        ...
        <div className="chart_boxes">
          <div className="admin-header">
            <h6 className="admin-head">Transaction Logs</h6>
          </div>
          <div className="chart_area_body">
            <div className="row">
              <div className="col-auto">
                <button className="btn ml-2" onClick={excelExport}>
                  <i className="fas fa-file-download"></i> Download Report
                </button>
              </div>
              <div className="col-auto ml-auto d-md-flex align-items-center">
                <form
                  className="common_search-type-1 form-half mx-2"
                  ...
                  <tbody>
                    {logs.map((log, index) => (
                      <tr key={index}>
                        <td>{log?.name}</td>
                        <td>{log?.userId}</td>
                        <td>{log?.type?.toLowerCase()}</td>
                        <td>{moment(log?.createdAt).format("MMM D, YYYY")}</td>
                        <td>₹ {log?.price}</td>
                      </tr>
                    ))}
                  </tbody>
