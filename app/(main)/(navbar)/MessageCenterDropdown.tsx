...
interface MessageCenterDropdownProps {
  user: any;
}

export function MessageCenterDropdown({ user }: MessageCenterDropdownProps) {
  ...
  const [params, setParams] = useState<{
    ...
    notIn: string;
    type: string;
  }>({ limit: 20, page: 1, notIn: "practicetest-mentor-sms", type: "message" });
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      setLoading(true);
      try {
        const res = await findAll({ ...params, userId: user?._id });
        setMessages(res);
        setHideLoadMore(res.length === params.limit);
        if (res.length) {
          openMessage(res[0]);
        }
      } catch (error) {
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      setHideLoadMore(result.length < params.limit);
    } catch (error) {
      setLoading(false);
  ...
  const remove = async (message: Message) => {
    if (message) {
      alertify.confirm(
        ...
        async (status: any) => {
          try {
            await removeByUser(message._id);
            setMessages((prevMessages) =>
              prevMessages.filter((m) => m._id !== message._id)
            );
            if (activeMsg && activeMsg._id === message._id) {
              setActiveMsg(null);
            }
            alertify.success("Message is removed successfully.");
          } catch (error) {
  ...
  return (
    <div>
      <img
        ref={iconRef}
        src="/assets/images/icon-msg-center-new.png"
        alt="Message Center"
        className={`message-center-icon ${showDropdown ? "active" : ""}`}
      ...
      {showDropdown && (
        <div
          className="message-center-dropdown"
          ref={dropdownRef}
          style={{
            position: "absolute",
            top: "100%",
        ...
        >
          ...
          >
            <h4 style={{ margin: 0 }}>Message Center</h4>
          </div>
          <div
          ...
          >
            {loading ? (
                ...
                {messages.map((msg) => (
                  <div
                    ...
                    className={`message-item ${
                      activeMsg && msg._id === activeMsg._id ? "active" : ""
                    }`}
              ...
              <div className="text-center">
                ...
                <h5 className="mt-2">No new message</h5>
              </div>
            )}
          </div>
          <div
...
export default MessageCenterDropdown;
