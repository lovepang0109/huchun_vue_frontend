...
import { faComment, faCopy, faCopyright } from '@fortawesome/free-regular-svg-icons';
...
export default function ClassBoardDetail() {
  ...
  const setQuestion = async (result: any) => {
  ...
  useEffect(() => {
    ...
    clientApi.get(`/api/questions/classboard/${code}`).then(res => {
      ...
      for (const usr of res.data.users) {
        ...
        if (usr.user._id == user?.info._id) {
          usr.joined = true
          usr.answers.forEach((ans: any) => {
            lockedAnswers[ans.id] = true
          });

          for (const a of res.data.question.answers) {
        ...
        if (user?.info?._id == res.data.user._id) {
          for (const ua of usr.answers) {
            for (let i = 0; i < res.data.question.answers.length; i++) {
              const a = res.data.question.answers[i]
              if (a._id == ua._id) {
      ...
      if (res.data.question.category == 'code') {
  ...
  const onUserAction = (data: any) => {
    if (data.code == classboard.code) {
      if (data.action == 'answer') {
        ...
        const fu = users.find((u: any) => u.user._id == user?.info._id)
  ...
  const selectAnswer = (answer: any) => {
    ...
    clientApi.put(`/api/questions/classboard/${code}/answer`, { answers: classboard.question.answers.filter((an: any) => an.isChecked).map((a: any) => { return { _id: a._id } }) }).then(res => {
      // this.socketSvc.emit("action.classboard", {
      //   action: 'answer',
      //   code: classboard.code,
      //   userId: user._id,
        ...
        <div className="code-editor">
          <div className="code-highlight">
            <div className="d-flex justify-content-between align-items-center">
              ...
              <div className="d-flex align-items-center">
                <a href={'/home'} className="btn btn-outline mr-2">Return</a>

                {classboard && <div className="mr-2">
                  ...
                  {classboard.question.category == 'mcq' && !classboard.showAnswer && user?.info._id == classboard.user._id && <button className="btn btn-primary mr-2" onClick={showAnswer}>Show Result</button>}
          ...
          {classboard && <div className="row no-gutters" >
            <div className="col-8">
              {classboard.question.category == 'mcq' && <div>
                ...
                {!classboard.showAnswer && <div className="border rounded p-3 m-3">
                  <McqQuestion question={classboard.question} setQuestion={setQuestion} answerChanged={selectAnswer} lockedAnswers={lockedAnswers}></McqQuestion>
                </div>}
            ...
            <div className="col-4">
              <div className="border rounded p-3 m-3">
                <h4>Participants</h4>
                <div className="mt-2">
                  {users && users.map((pu: any) => <div key={pu._id} className="mt-2">
                    <div className="d-flex align-items-center">
                      <figure className="user_img_circled_wrap">
                        <img src='/assets/images/defaultProfile.png' />
                        {/* <div className="avatar" style={{ backgroundImage: 'url(&quot;/assets/images/defaultProfile.png&quot;);' }}></div> */}
                      </figure>
                      <div>
                        <div className="d-flex align-items-center">
                          <h5 className="mx-2">{pu.user.name}</h5>
                          {pu.user._id != user?.info._id && <a onClick={() => openChat(pu.user)}><FontAwesomeIcon icon={faComment} /></a>}
                        </div>
                        {classboard.question.category == 'mcq' && user?.info._id == classboard.user._id && pu.answers?.length && <div>
                        ...
                        {classboard.question.category == 'code' && user?.info._id == classboard.user._id && pu.user._id != user?.info._id && <div className="mt-1" >
