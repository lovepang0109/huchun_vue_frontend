...
export default function Analytics() {
  ...
  const [questionSubjectDistributionArr, setQuestionSubjectDistributionArr] = useState<any[]>([]);
  const [allQuestionDistribution, setAllQuestionDistribution] = useState<any>([]);
  const [selectedQuestionDistribution, setSelectedQuestionDistribution] = useState<string>('');
  const [assssmentSubArr, setAssssmentSubArr] = useState<any>([]);
  const [allAssessmentData, setAllAssessmentData] = useState<any>([]);
  const [selectedAssessmentSubject, setSelectedAssessmentSubject] = useState<string>('');
  ...
  const [questionCategoryDistributionChartOptions, setQuestionCategoryDistributionChartOptions] = useState<any>({
    series: [],
    options: {
      chart: {
        type: "bar",
  ...
  const [assessmentWiseChartOptions, setAssessmentWiseChartOptions] = useState<any>({
    series: [],
    options: {
      chart: {
        // type: "bar",
  ...
  useEffect(() => {
    ...
    clientApi.get(`/api/testSeries/percentCompleteTestseries/${id}`).then(data => {
      setCompletionData(data.data);
    });
    clientApi.get(`/api/testSeries/percentAccuracyTestseries/${id}`).then(data => {
      console.log(data, "accuracy>>>>>>><<<<<<<<<<<<")
      setAccuracyData(data.data);
    });
    clientApi.get(`/api/testSeries/practiceHoursTestSeries/${id}`).then(data => {
      setHoursData(data.data);
    });
    clientApi.get(`/api/testSeries/percentCompleteTestseries/${id}${toQueryString({
      limit: '15days',
    })
      }`).then(data => {
        setIntervalCompletionData(data.data);
      })
    clientApi.get(`/api/testSeries/percentCompleteTestseries/${id}${toQueryString({ daysLimit: 15 })}`).then(data => {
      console.log(data.data, "accuracy>>>>>>>>>>>>>>>>>")
      setIntervalAccuracyData(data.data);
    })
    clientApi.get(`/api/testSeries/practiceHoursTestSeries/${id}${toQueryString({ limit: '15days' })}`).then(data => {

      setIntervalHoursData(data.data);
    })
    clientApi.get(`/api/testSeries/questionCategoryTestSeries/${id}`).then(async (d: any) => {
      ...
      if (d.data && d.data.length) {
        d.data.forEach((element: any) => {
          if (!temp.some(sub => sub._id === element.subjectId)) {
            setQuestionSubjectDistributionArr([...questionSubjectDistributionArr, { _id: element.subjectId, name: element.subjectName }]);
            temp = [...questionSubjectDistributionArr, { _id: element.subjectId, name: element.subjectName }]

          }
        });
        await setSelectedQuestionDistribution(temp[0]?.name);
        const filterData = d.data.filter((sub: any) => sub.subjectName.toLowerCase() === temp[0]?.name.toLowerCase());
        const data = filterData;
        drawQuestionCategoryDistribution(data, false)
      }
    })
    clientApi.get(`/api/testSeries/assesmentWiseMarksTestSeries/${id}`).then((d: any) => {
      ...
      d.data.user.forEach((element: any) => {
        if (!temp.some((sub: any) => sub._id === element.subjectId)) {
          setAssssmentSubArr([...assssmentSubArr, { _id: element.subjectId, name: element.subjectName }]);
          temp = [...assssmentSubArr, { _id: element.subjectId, name: element.subjectName }]
        }
      });
      if (temp && temp.length > 0) {
        setSelectedAssessmentSubject(temp[0].name);
        const userData = d.data.user.filter((sub: any) => sub.subjectName.toLowerCase() === temp[0].name.toLowerCase());
        const averageData = d.data.average.filter((sub: any) => sub.subjectName.toLowerCase() === temp[0].name.toLowerCase());
        const topperData = d.data.topper.filter((sub: any) => sub.subjectName.toLowerCase() === temp[0].name.toLowerCase());


        drawAssessmetWiseGraph(userData, averageData, topperData, false);
      }
    })
    clientApi.get(`/api/testSeries/subjectWiseMarksTestSeries/${id}`).then((data: any) => {
      setAllSubjectData(data.data);
      let temp = subjectArr
      data.data.user.forEach((element: any) => {
        if (!temp.some((sub: any) => sub._id === element?.subjectId)) {
          setSubjectArr([...subjectArr, { _id: element?.subjectId, name: element?.subjectName }]);
          temp = [...subjectArr, { _id: element?.subjectId, name: element?.subjectName }]
        }
      });
      if (temp && temp.length > 0) {
        setSelectedSubject(temp[0].name);
        const userData = data.data.user.filter((sub: any) => sub?.subjectName.toLowerCase() === temp[0].name.toLowerCase());
        const averageData = data.data.average.filter((sub: any) => sub?.subjectName.toLowerCase() === temp[0].name.toLowerCase());
        const topperData = data.data.topper.filter((sub: any) => sub?.subjectName.toLowerCase() === temp[0].name.toLowerCase());
        drawSubjectWiseGraph(userData, averageData, topperData, false)
      }
    })
  }, []);

  const drawAssessmetWiseGraph = (user: any, average: any, topper: any, reload: any) => {
  ...
  const filterQuestionDistribution = (filter: any) => {
    ...
    const filterData = allQuestionDistribution.filter(sub => sub.subjectName.toLowerCase() === filter.toLowerCase());
    const data = filterData;
    drawQuestionCategoryDistribution(data, true);
  }

  const drawSubjectWiseGraph = (user: any, average: any, topper: any, reload: any) => {
    const averageMarks: any = [];
    const userMarks: any = [];
    const topperMarks: any = [];
  ...
  const filterSubjectWise = (name: any) => {
    ...
    const userData = allSubjectData.user.filter((sub: any) => sub.subjectName.toLowerCase() === name.toLowerCase());
    const averageData = allSubjectData.average.filter((sub: any) => sub.subjectName.toLowerCase() === name.toLowerCase());
    const topperData = allSubjectData.topper.filter((sub: any) => sub.subjectName.toLowerCase() === name.toLowerCase());
  ...
  const filterAssessmentWise = (name: any) => {
    ...
    const userData = allAssessmentData.user?.filter((sub: any) => sub.subjectName.toLowerCase() === name.toLowerCase());
    const averageData = allAssessmentData.average?.filter((sub: any) => sub.subjectName.toLowerCase() === name.toLowerCase());
    const topperData = allAssessmentData.topper?.filter((sub: any) => sub.subjectName.toLowerCase() === name.toLowerCase());
            ...
            <div className="container5 bg-white">
              ...
              <div className="overall-main">
                <div className="row">
                    ...
                    <div className="admin-box data_ana_box">
                      ...
                      <div className="form-row">
                        <div className="col-4">
                          ...
                          <p
                            className="over3 d-flex align-items-center justify-content-center"
                          >
                        ...
                        <div className="col-4">
                          ...
                          <h4 className="over2-inactive">
                            {
                              Math.round(completionData?.average?.percentComplete)}%
                          </h4>
                          <p
                            className="over3 d-flex align-items-center justify-content-center"
                          >
                        ...
                        <div className="col-4">
                          ...
                          <h4 className="over2-inactive">
                            {
                              Math.round(completionData?.topper?.percentComplete)}%
                          </h4>
                          <p
                            className="over3 d-flex align-items-center justify-content-center"
                          >
                  ...
                  {accuracyData && accuracyData.user && <div className="col-lg-4">
                    <div className="admin-box data_ana_box">
                      ...
                      <div className="form-row">
                        <div className="col-4">
                          ...
                          {intervalAccuracyData && intervalAccuracyData.user && <p
                            className="over3 d-flex align-items-center justify-content-center"
                          >
                            <FontAwesomeIcon icon={faSortUp} className="pt-1"> arrow_drop_up </FontAwesomeIcon>
                            <span className="persist-arrow ml-1"
                        ...
                        <div className="col-4">
                          <p className="over1">Average</p>
                          <h4 className="over2-inactive">
                            {Math.round(accuracyData.average.accuracy ? accuracyData.average.accuracy : 0)} %
                          </h4>
                          {intervalAccuracyData && intervalAccuracyData.average && <p
                            className="over3 d-flex align-items-center justify-content-center"
                          >
                            <FontAwesomeIcon icon={faSortUp} className="pt-1"> arrow_drop_up </FontAwesomeIcon>
                            <span className="persist-arrow ml-1"
                            >{
                                Math.ceil(intervalAccuracyData.average.percentComplete ? intervalCompletionData.average.percentComplete + intervalAccuracyData.average.percentComplete : 0)}%</span>
                          </p>}
                        </div>
                        <div className="col-4">
                          <p className="over1">Topper</p>
                          ...
                          {intervalAccuracyData && intervalAccuracyData.topper && <p
                            className="over3 d-flex align-items-center justify-content-center">
                            <FontAwesomeIcon icon={faSortUp} className="pt-1"> arrow_drop_up </FontAwesomeIcon>
                            <span className="persist-arrow ml-1"
                            >{
                      ...
                      <p className="over-0 text-left">*Accuracy in last 15 days</p>
                    </div>
                  </div>}

                  {hoursData && hoursData.user && <div className="col-lg-4">
                    <div className="admin-box data_ana_box">
                      <div className="overall-head text-center">Practice Hour(s)</div>
                      <div className="form-row">
                        <div className="col-4">
                          <p className="over1">You</p>
                          <h4 className="over2">
                          ...
                          {intervalHoursData && intervalHoursData.user && <p
                            className="over3 d-flex align-items-center justify-content-center"
                          >
                            <FontAwesomeIcon icon={faSortUp} className="pt-1"> arrow_drop_up </FontAwesomeIcon>
                            <span className="persist-arrow ml-1"
                        ...
                        <div className="col-4">
                          ...
                          {intervalHoursData && intervalHoursData.average && <p
                            className="over3 d-flex align-items-center justify-content-center"
                          >
                            <FontAwesomeIcon icon={faSortUp} className="pt-1">arrow_drop_up </FontAwesomeIcon>
                            <span className="persist-arrow ml-1"
                        ...
                        <div className="col-4">
                          ...
                          {intervalHoursData && intervalHoursData.topper && <p
                            className="over3 d-flex align-items-center justify-content-center"
                          >
                            <FontAwesomeIcon icon={faSortUp} className="pt-1">arrow_drop_up</FontAwesomeIcon>
                            <span className="persist-arrow ml-1"
                      ...
                      <p className="over-0 text-left">*Time Spend in last 15 days</p>
                    </div >
                  </div >}
                </div >
                <div className="chart_boxes admin-box2">
                  <div className="test-series-analytics">
                    <div className="admin-header p-8">
                      <h4 className="admin-head line-height-initial">Performance Breakdown by Subject</h4>
                      <p className="admin-head2">
                        This chart lets you see your marks in each subject alongside the average and top performers&apos;s scores. Use the Dropdown menu to switch between subjects and directly compare your performance. <br /> It&apos;s a great way to identify where you&apos;re shining and which areas need more focus
                      </p>
                    </div>
                    <div className="chart_area_body">
                      {subjectArr && subjectArr.length > 0 &&
                        <div className="filter-area clearfix mb-5">
                          <div className="filter-item">
                            <select className="form-control" name="subject" value={selectedSubject.name} required onChange={(e) => filterSubjectWise(e.target.value)}>
                              <option value={''} disabled>Select</option>
                              {
                                subjectArr.map((s: any, index: any) => {
                                  return <option value={s.name} key={index}>
                                    {s.name}
                                  </option>
                                })
                              }
                      ...
                      {subjectWiseChartOptions.series && subjectWiseChartOptions.series.length > 0 && <Chart type="bar" height={300}
                        series={subjectWiseChartOptions.series}
                        options={subjectWiseChartOptions.options}
                      >
                      </Chart>}
                      {subjectArr && subjectArr.length == 0 && <div className="row">
                        <div className="col-5"></div>
                        <div className="col-2 NoGrapghData addNoDataFullpageImgs1">
                          <figure className="ml-5" style={{ height: '80px', width: '60px' }}>
                            <img
                              src="/assets/images/noSubject.svg"
                              alt="Picture for No Subject"
                            />
                          </figure>
                          <h4 className="mt-2" style={{ marginLeft: '20%' }}>No data yet!</h4>
                        </div>
                      </div >}
                    </div >
                  </div >
                ...
                <div className="chart_boxes admin-box2">
                  <div className="test-series-analytics">
                    <div className="admin-header">
                      <h4 className="admin-head line-height-initial">Assesment Wise Result</h4>
                      <p className="admin-head2">
                        You will know about your performance in Assesment
                      </p>
                    </div>
                    <div className="chart_area_body">
                      {assssmentSubArr && assssmentSubArr.length > 0 &&
                        <div className="filter-area clearfix mb-5">
                          <div className="filter-item">
                            <select className="form-control" name="subject" value={selectedAssessmentSubject.name} required onChange={(e) => filterAssessmentWise(e.target.value)}>
                              <option value={''} disabled>Select</option>
                              {
                      ...
                      {assessmentWiseChartOptions.series &&
                        assessmentWiseChartOptions.series.length > 0 && <Chart type="bar" height={300}
                      ...
                      {assssmentSubArr && assssmentSubArr.length == 0 && <div
                        ...
                        <div className="col-5"></div>
                        <div className="col-2 NoGrapghData addNoDataFullpageImgs1">
                          <figure className="ml-5" style={{ height: '65px', width: '60px' }}>
                            <img
                ...
                <div className="row">
                  <div className="col-lg-5">
                    <div className="chart_boxes admin-box3-remove h-lg-100">
                      <div className="admin-header">
                        <h4 className="admin-head line-height-initial">Overall Rank</h4>
                        <p className="admin-head2">
                          Know how you performed in the test series
                        </p>
                      </div>
                      <div className="chart_area_body">
                        {studentsRanks && studentsRanks.length > 0 && <div
                          className="admin-box-info">
                          {studentsRanks.map((s: any, i: any) => <div key={i}>
                            <div
                              className="form-row align-items-center py-2"
                            >
                              ...
                              <div className="col-auto">
                                <div className="admin-info2-dir score_wrap">
                                  {i == 0 && <p className="over-table-head">Score</p>}
                                  <h6>{s.marks}</h6>
                                </div>
                              </div>
                            </div>
                        ...
                        {studentsRanks.length == 0 && <div
                          className="row">
                          <div className="col-3"></div>
                          <div className="col-8 pl-0 NoGrapghData addNoDataFullpageImgs1">
                            <figure className="mt-4" style={{ height: '225px', width: '260px' }}>
                              <img
                                src="/assets/images/noRankk.svg"
                                alt="Picture for noRank"
                              />
                            </figure>
                            <h4 className="mt-2" style={{ marginLeft: '30%' }}>
                              No data yet!
                            </h4>
                          </div>
                        </div >}
                  ...
                  <div className="col-lg-7">
                    <div className="chart_boxes admin-box3-remove h-lg-100">
                      <div className="test-series-analytics-1">
                        <div className="admin-header">
                          <h4 className="admin-head line-height-initial">Question Category-wise Distribution</h4>
                          <p className="admin-head2">
                            This chart breaks down how you&apos;ve answered different categories of questions. You can see the number of questions you got right, wrong, or didn&apos;t attempt for each question category. Use the subject dropdown to focus on specific subjects and better understand your strengths and areas for improvement.
                          </p>
                        </div>
                        {selectedQuestionDistribution && <div
                          className="chart_area_body">
                          {questionCategoryDistributionChartOptions &&
                            questionCategoryDistributionChartOptions.series && <Chart type="bar"
                              series={questionCategoryDistributionChartOptions.series}
                              options={questionCategoryDistributionChartOptions.options}
                            >
                            </Chart>}
                          <div className="filter-area clearfix">
                            <div className="filter-item">
                              <div className="dropdown">
                                ...
                                >
                                  {questionSubjectDistributionArr.map((q: any, i: any) => <a key={i}
                                    className="dropdown-item"
                                    onClick={() => filterQuestionDistribution(q.name)}
                                  > {q.name}</a>)}
                                </div>
                              </div>
                            </div>
                        ...
                        {!selectedQuestionDistribution && <div
                          className="chart_area_body text-center">
                          <svg
                            width="344"
                            height="308"
                            viewBox="0 0 344 308"
