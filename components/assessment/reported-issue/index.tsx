...
import { replaceBadWords } from "@/lib/helpers";
...
const ReportedIssueComponent = ({
  ...
  const [includeResolved, setIncludeResolved] = useState<boolean>(true);
  ...
  useEffect(() => {
    const fetchFunc = async () => {
      return await authSvc.getToken();
    };
    const token = fetchFunc();
    ...
    feedbackSvc
      .getQuestionFeedbacks(temp_params)
      .then((result: any[]) => {
        const temp_ques = processQuestionData(result);
        setQuestions(temp_ques);
        console.log("temp_ques", temp_ques);
        filterQuestions(temp_ques);
      })
      .catch((err: any) => {
        console.log(err);
      });
  ...
  //modal
  const openRespondModal = (question, feedback?) => {
  ...
  const postDiscussion = async () => {
    ...
    discussionService
      ...
      .then((data: any) => {
        cancel();
        feedbackSvc.getQuestionFeedbacks(params).then((result: any) => {
          if (result?.length === 0) {
            return;
          }
          result.map((r: any) => {
            r.feedback.map((element: any) => {
              if (element?.feedbackComments?.length > 0) {
                element.feedbackComments.map((fc: any) => {
                  element.comment = element.comment + "<br>" + fc;
                });
              }
            });
          });
  ...
  const sendMail = () => {
    ...
    currentFeedback.forEach((element) => {
      dataToSave.studentQ.push({
        studentId: element.studentId,
        feedbackId: element.feedbackId,
      });
    ...
    feedbackSvc
      ...
      .then((da) => {
        ...
        feedbackSvc.getQuestionFeedbacks(params).then((result: any[]) => {
          ...
          const questions_temp = processQuestionData(result);
  ...
  const chat = () => {
    if (!feedback.chatComment) {
      return;
    }
    // send chat message
    for (const element of currentFeedback) {
      if (element.studentId != user._id) {
        chatSvc
          .getRoom(element.studentId, element.studentName)
          .then((chatIns: any) => {
            const msg = `You have reported an issue of question ${
              currentQues.order
            } \
          in the assessment ${
            currentQues.test.title
          }: ${element.comment.replace(/<br>/g, "")}`;
            // chatSvc.send(chatIns.uid, msg, user);

            const comment = replaceBadWords(feedback.chatComment); // Assuming replaceBadWords is defined
            setTimeout(() => {
              // chatSvc.send(chatIns.uid, `Response: ${comment}`, user);
            }, 1000);
          });
      } else {
        error("Cannot send chat message to your own feedback.");
        return;
      }
    }

    success("Chat message is sent successfully.");
    ...
    const dataToSave = {
      ...
      studentQ: [] as StudentQ[],
      teacherR: {
        comment: replaceBadWords(feedback.chatComment),
      },
      test: currentQues.test.title,
    };
  ...
  const processQuestionData = (questions: any[]) => {
    setStats({
      ...stats,
    ...
    questions.map((r: any) => {
      ...
      for (const fb of r.feedback) {
        if (fb.feedbackComments.length > 0) {
          fb.feedbackComments.map((fc: any) => {
  ...
  const openQuestion = (question: any) => {
    ...
    router.push(
      `/question/create?questionId=${question._id}&testId=${test._id}&isEdit='edit'&testMenu='issues'`
    );

    // router.push(`/question/create/${question._id}`, queryParams);
  };

  const resolveQuestion = (question: any) => {
    // setIncludeResolved(!includeResolved)
    feedbackSvc.markQuestionResolved(question._id).then(() => {
      question.unsolved = false;
      setStats((prevStats: any) => ({
        ...prevStats,
        resolved: prevStats.resolved + 1,
      }));
      const temp_includeResolved = includeResolved;

      if (question?.length > 0) {
        const updatedFilteredQuestions = temp_includeResolved
          ? [...q]
          : q?.filter((qu) => qu.unsolved);
        setFilteredQuestions(updatedFilteredQuestions);
      } else {
        const updatedFilteredQuestions = temp_includeResolved
          ? [...questions]
          : questions.filter((q) => q.unsolved);
        setFilteredQuestions(updatedFilteredQuestions);
      }
  ...
  const handleRadioChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setFeedbackOptions(e.target.value);
  };

  const filterQuestions = (q?: any) => {
    setIncludeResolved(!includeResolved);
    const temp_includeResolved = !includeResolved;
    ...
    <>
      {questions && questions.length > 0 && (
        <div className="assessment-settings non-stepper">
          <div className="bg-white rounded-boxes mb-3 d-flex justify-content-between align-items-center text-center f-16">
            ...
            <div className="switch-item float-none">
              <div className="d-flex align-items-center">
                <span className="mr-3 switch-item-label">Include Resolved</span>
            ...
            {/* <app-toggle label="Include Resolved" [(value)]="includeResolved" (valueChange)="filterQuestions()"></app-toggle> */}
          </div>
                  ...
                  <>
                    <div className="d-flex justify-content-between">
                      <span>
                        ...
                        {timeAgo(question.questionUpdatedDate)}{" "}
                    ...
                    <div className="content bg-white mt-2">
                      <div className="asses-rights clearfix border-0 py-0">
                        {question.feedback.map(
                          (qFeedback: any, feedbackIndex: any) => (
                            <span key={feedbackIndex}>
                              <h4 className="mb-2">
                                <strong className="byStdNameAllStrng-1">
                                  {" "}
                                  By {qFeedback.studentName}
                                </strong>{" "}
                                ({timeAgo(qFeedback.createdAt)}){" "}
                                {/* Convert to appropriate format */}
                              </h4>
                              <div className="row">
            ...
            <div className="d-flex align-items-center">
              ...
              <div className="container1 ml-3">
                <input
                  ...
                  style={{ appearance: "radio" }}
                />
                <label htmlFor="rdDiscussion"></label>
              </div>
...
export default ReportedIssueComponent;
